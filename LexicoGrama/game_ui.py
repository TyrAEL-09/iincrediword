...
class GameUI:
    ...
    def __init__(self, root: tk.Tk, ui_settings, on_cell_click, on_submit_word, on_pause_toggle, on_music_toggle, on_clear_selection):
        self.root = root
    ...
    def show_game_interface(self):
        ...
        # Establecer el tamaño de la ventana principal (root)
        # Por ejemplo, 750x750 para que sea cuadrada y un poco más pequeña.
        # Puedes ajustar este valor si lo deseas.
        desired_width = 750
        desired_height = 750
        
        # Opcional: Centrar la ventana en la pantalla.
        # Si deseas centrar la ventana, puedes descomentar las siguientes líneas:
        ...
        # Si solo quieres establecer el tamaño sin centrar explícitamente aquí:
        self.root.geometry(f"{desired_width}x{desired_height}")
        
        # Opcional: Evitar que el usuario redimensione la ventana del juego
        # para mantener la forma cuadrada. Si activas esto, considera revertirlo
        # en MenuUI.show_menu_interface() con self.root.resizable(True, True)
        # para que el menú principal sí sea redimensionable si así lo deseas.
        ...
        self.button_clear.config(state=NORMAL) # Asegurar que el botón limpiar esté habilitado
        self.button_pause_game.config(state=NORMAL, text="Pausar") # Asegurar texto y estado correctos
    ...
    def hide_game_interface(self):
        ...
        # Opcional: Si hiciste la ventana no redimensionable,
        # aquí podrías hacerla redimensionable de nuevo si es necesario
        # antes de volver al menú, o manejarlo en MenuUI.show_menu_interface().
    ...
    def update_board_selection(self, current_selection_path: List[Tuple[int, int]]):
        ...
        for r_idx, c_idx in current_selection_path:
            if 0 <= r_idx < len(self.grid_cell_widgets) and 0 <= c_idx < len(self.grid_cell_widgets[0]): # Check bounds
                if self.grid_cell_widgets[r_idx][c_idx]:
        ...
        for length in sorted(words_by_length.keys(), reverse=True):
            ...
            for word_str in sorted(words_by_length[length]):
                ...
                for idx, char_in_palabra in enumerate(word_str):
                    ...
                    if not is_found and idx == 0: # Mostrar la primera letra como pista
    ...
    def update_found_word_display(self, word: str):
        ...
        word_upper = word.upper() # Asegurar que la clave sea mayúscula
    ...
    def disable_game_controls(self):
        ...
        # self.button_pause_game.config(state=NORMAL, text="Reanudar") # Corrección: el botón Pausa/Reanudar debe seguir NORMAL para poder clickearlo.
        self.button_pause_game.config(text="Reanudar") # Solo cambia el texto, el estado debe permitir click
        if self.grid_cell_widgets:
            for row_widgets in self.grid_cell_widgets:
                for widget_cell in row_widgets:
